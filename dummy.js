const data = {
 ...
 function getCarModels(make) {
   //const uniqueArray = Array.from(new Set(data[make].map(item => item)));//
   return Array.from(new Set(data[make].map(item => item.model.trim()))).map(model => ({ model }));

}

// Array of keys
// Array of key-value pairs in the desired format
 const carDataFormatted= Object.keys(data).map((make) => ({
  key: make,
  value: make,
}));
...
function eliminateDuplicates(data) {
  ...
  for (const brand in data) {
    if (data.hasOwnProperty(brand)) {
      ...
      data[brand].forEach((car) => {
        const model = car.model.trim();
        if (!uniqueModelsSet.has(model)) {
          uniqueModelsSet.add(model);
          uniqueModelsArray.push({ model });
        }
      });

      result[brand] = uniqueModelsArray;
    }
  }
...
function createJsonFile(data, fileName) {
  ...
  fs.writeFile(fileName, jsonData, 'utf8', (err) => {
    if (err) {
      ...
      console.log('JSON file has been created successfully:', fileName);
    }
  });
}
...
const dataWithoutDuplicates = eliminateDuplicates(data);
createJsonFile(dataWithoutDuplicates, 'output.json');
